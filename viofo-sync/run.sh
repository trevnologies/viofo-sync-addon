...
UI_NOTIFICATIONS=$(jq -r '.ui_notifications' "$OPTIONS_FILE")
UI_NOTIFY_CONFIG_CHANGE=$(jq -r '.ui_notify_config_change' "$OPTIONS_FILE")
SYNC_ON_STARTUP=$(jq -r '.sync_on_startup' "$OPTIONS_FILE")
SKIP_STARTUP_WHEN_AWAY=$(jq -r '.skip_startup_sync_when_away' "$OPTIONS_FILE")
